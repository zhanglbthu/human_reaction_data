...
def plot_3d_motion(save_path, kinematic_tree, joints, title, figsize=(10, 10), fps=120, radius=4):
    ...
    def update(index):
        ...
        xx, zz = np.meshgrid(np.linspace(MINS[0], MAXS[0], 10),
                     np.linspace(MINS[2], MAXS[2], 10))
        yy = np.zeros_like(xx)  # 地面 y=0
        ax.plot_wireframe(xx, yy, zz, color='gray', alpha=0.3)  # 网格地面
        
        ax.scatter(data[index, :22, 0], data[index, :22, 1], data[index, :22, 2], color='black', s=3)

        if index > 1:
            ax.plot3D(trajec[:index+1, 0],
                      np.zeros_like(trajec[:index+1, 0]),
                      trajec[:index+1, 1], linewidth=1.0,
                      color='blue')
